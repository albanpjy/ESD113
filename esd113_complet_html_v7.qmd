...
* 📄 **Format PDF :** [Télécharger le rapport PDF](https://albanpjy.github.io/ESD113/esd113_complet_plus_18_pdf_posit.pdf)
:::
---

# Introduction générale {.unnumbered #sec-intro-generale}

Ce document a été réalisé dans le cadre de l'unité d'enseignement *ESD113-Probabilités et statistiques avec R*, dispensée au Conservatoire National des Arts et Métiers (CNAM) par Monsieur Karim KILANI, il constitue un rapport de référence méthodologique. Chaque concept y est présenté avec la rigueur : définitions formelles, justifications, implémentation en R et interprétation critique des résultats. Un recours significatif aux outils d'intelligence artificielle a permis de résoudre les problématiques complexes de mise en page avec précision et rapidité, tout en facilitant la structuration du contenu.contenu.
...
Pour chaque formule ci-dessous, le bloc grisé montre le *code LaTeX source* tel qu'il apparaît dans le document `.qmd`, et le rendu est affiché juste en dessous. Cette double présentation est précieuse pour l'apprentissage : on voit à la fois ce qu'on écrit et ce que MathJax produit.
...
::: {.callout-tip title="À retenir"}
Le modèle sélectionné par `mclust` expose son $d$ via `mc_model$df` en R.
