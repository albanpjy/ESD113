...
# Lire le KNITR_TOTAL de la session précédente AVANT d'écraser le fichier
.prev_knitr_total_html <- tryCatch({
  lignes_prev <- readLines("compilation_stats_html.txt")
...
assign(".prev_knitr_total_html", .prev_knitr_total_html, envir = .GlobalEnv)
...
<div style="text-align: right; margin-top: 1.5em; font-style: italic; color: #555;">
*Alban VIDELOUP*<br>
*Paris, `r withr::with_locale(c(LC_TIME = "fr_FR.UTF-8"), format(Sys.Date(), "%d %B %Y"))`*
</div>

::: {.callout-tip}
### 🔗 Accès aux ressources du projet
Ce rapport fait partie d'un projet complet sur les statistiques et le clustering. Vous pouvez accéder aux différentes versions ici :

* 🌐 **Version Web interactive :** [Publication RPubs](https://rpubs.com/Albanpjy/esd113_complet_html_v7)
* 💻 **Code source & Projet :** [Dépôt GitHub ESD113](https://albanpjy.github.io/ESD113/)
* 📄 **Format PDF :** [Télécharger le rapport PDF](https://albanpjy.github.io/ESD113/esd113_complet_plus_19_pdf_posit.pdf)
:::
---

# Introduction générale {.unnumbered #sec-intro-generale}

Ce document a été réalisé dans le cadre de l'unité d'enseignement *ESD113-Probabilités et statistique avec R*, dispensée au Conservatoire National des Arts et Métiers (CNAM) par Monsieur Karim KILANI. Il constitue un rapport de référence méthodologique. Chaque concept y est présenté avec la rigueur d'un manuel académique : définitions formelles, justifications, implémentation en R et interprétation critique des résultats. Un recours significatif aux outils d'intelligence artificielle a permis de résoudre les problématiques complexes de mise en page avec précision et rapidité, tout en facilitant la structuration du contenu.

Ce rapport fusionne trois volets complémentaires du cours :
...
::: {.callout-tip}
## Note sur le choix du format HTML
Ce document est la version HTML du rapport. Si le PDF fait appel à LaTeX pour la mise en forme typographique (boîtes, tableaux, formules avancées), la version HTML exploite les callouts Quarto, MathJax et les outils `htmltools` pour un rendu interactif et responsive. Les formules mathématiques affichent ici à la fois leur *code source LaTeX* (pour l'apprentissage) et leur *rendu MathJax*. Le couple Quarto + HTML constitue le meilleur des deux mondes : lisibilité du Markdown, puissance de MathJax, et navigabilité hypertexte.
:::
...
Quarto repose sur la syntaxe *Markdown* pour la mise en forme du texte. On peut écrire du texte en **gras** et en *italique*, insérer des listes, des tableaux, et bien sûr des formules mathématiques grâce à $\LaTeX$. Passons en revue les commandes Markdown les plus utiles, selon la même convention pédagogique que pour les formules mathématiques présentées plus bas : chaque bloc grisé montre le *code source Markdown* tel qu'il est tapé dans le fichier `.qmd`, et le rendu produit s'affiche juste en dessous.
...
Pour les formules mathématiques, Markdown délègue à $\LaTeX$ : tout ce qui est placé entre `$...$` (en ligne) ou `$$...$$` (centré) est interprété comme du code mathématique. Pour chaque formule ci-dessous, le bloc grisé montre le *code LaTeX source* tel qu'il apparaît dans le document `.qmd`, et le rendu est affiché juste en dessous. Cette double présentation est précieuse pour l'apprentissage : on voit à la fois ce qu'on écrit et ce que MathJax produit.
...
Le jeu de données intégré `mtcars` (32 modèles d'automobiles, 11 variables) illustre les opérations d'indexation fondamentales en R : accès par nom de colonne avec `$`, sélection par indices `[ligne, colonne]`, extraction d'une ligne entière ou d'une cellule individuelle. Ces quatre notations couvrent l'essentiel des besoins d'exploration d'un tableau en R de base, sans nécessiter de package Tidyverse.
...
La fonction `summarise()` de `dplyr` calcule des statistiques agrégées sur une ou plusieurs colonnes. Combinée au pipe `|>`, elle permet d'enchaîner sélection et résumé en une expression concise et lisible. Le tableau ci-dessous présente quatre indicateurs clés pour la variable `mpg` (consommation en miles par gallon) du jeu `mtcars`.
...
La fonction `hist()` de R de base produit un histogramme en une seule ligne de code. Elle accepte directement un vecteur numérique et offre les paramètres `main` (titre), `xlab` (axe des abscisses) et `col` (couleur des barres) pour personnaliser le rendu sans charger de bibliothèque supplémentaire.
...
::: {.callout-warning}
### Point d'attention
En version HTML, le thème ggplot2 est configuré globalement dans le chunk `setup` via `theme_set()`. Il n'est pas nécessaire d'ajouter `+ theme_minimal()` à chaque graphique-le thème s'applique automatiquement à tous les plots du document.
:::

### Exemple de représentation graphique en 3D avec Plotly

La bibliothèque `plotly` permet de créer des graphiques interactifs directement dans le navigateur, sans plugin. L'exemple ci-dessous construit un *histogramme 3D* à partir de 500 points tirés selon une loi normale bivariée $\mathcal{N}(0, I_2)$ : le plan $(x, y)$ est découpé en une grille $11 \times 11$, et chaque cellule est représentée par une barre 3D dont la hauteur correspond à l'effectif observé. La fonction auxiliaire `add_3Dbar()` construit chaque barre comme un prisme solide (`mesh3d`) défini par ses 8 sommets. Le graphique obtenu est *rotatif*, *zoomable* et *survol-actif* (infobulles au passage de la souris) — des fonctionnalités uniquement disponibles dans la version HTML du document.

::: {.callout-note}
### Version PDF
Cette visualisation interactive n'est pas disponible dans la version PDF. Consulter la version HTML publiée sur [albanpjy.github.io/ESD113/](https://albanpjy.github.io/ESD113/) pour interagir avec le graphique.
:::
...
*Calcul du pourcentage de présence.* Pour chaque numéro $i \in \{1, \ldots, 56\}$, on note $S_i$ le nombre de tirages dans lesquels il est sorti. Avec $N$ tirages historiques disponibles, le pourcentage de présence du numéro $i$ est défini par :

$$\mathrm{Pct}_i = \frac{S_i}{N} \times 100$$
...
$$p_{\text{théo}} = \frac{16}{56} \approx 0{,}2857 = 28{,}57\,\%$$

*Lecture du tableau.* Un numéro affichant un pourcentage proche de *28,57 %* est conforme à la loi théorique. Un écart vers le haut indique un numéro *sur-représenté* sur la période, vers le bas un numéro *sous-représenté*. Ces écarts relèvent de la fluctuation d'échantillonnage-ils n'ont aucune valeur prédictive pour les tirages futurs.
:::
...
::: {.callout-note}
### Rappel théorique
Le Keno est un tirage *sans remise* : les boules ne sont pas replacées dans l'urne. La distribution de probabilité adaptée est la *loi hypergéométrique*.
...
Toutes les barres sont *sous la ligne pointillée rouge* (la mise de 1 €), confirmant que l'*espérance de gain est toujours négative pour le joueur*, quelle que soit la grille choisie. Le TRJ affiché au-dessus de chaque barre (en % de la mise) montre que la FDJ reverse entre 47 % et 55 % de la mise en gains — la différence constituant la marge opératrice.
:::
...
Imaginez une carte sur laquelle figurent des centaines de villes représentées uniquement par leurs coordonnées géographiques (latitude, longitude) — deux variables *quantitatives continues*. Votre mission : regrouper ces villes en régions *sans connaître à l'avance* les frontières administratives. Naturellement, vous mesureriez les distances physiques et réuniriez les villes proches les unes des autres. Sans le savoir, vous appliquez le principe de l'algorithme K-means : minimiser la distance euclidienne entre chaque point et le centre de son groupe (le centroïde, calculé comme la *moyenne arithmétique* des coordonnées, axe par axe). C'est précisément parce que K-means repose sur ce calcul de distance que les variables doivent être *quantitatives et continues* — des attributs qualitatifs (couleur, catégorie) seraient incompatibles avec cette logique.

En statistique, le *clustering* (ou *classification non supervisée*) consiste à regrouper automatiquement des observations similaires *sans connaître à l'avance* les étiquettes ou catégories.
...
Évaluer une méthode de clustering sur des données réelles pose un problème fondamental : on ne connaît pas les *vrais* groupes, puisque c'est précisément ce qu'on cherche. La simulation résout ce problème en créant un monde artificiel où *la vérité est connue à l'avance*.

::: {.callout-tip}
### À retenir
La simulation est le mannequin de crash-test de la statistique : on connaît exactement les forces appliquées, et on mesure comment chaque algorithme s'en sort.
:::
...
::: {.callout-note}
### Notation : vecteurs et matrices en gras

À partir de cette section, les vecteurs et matrices sont notés en *gras* pour les distinguer des scalaires. Dans ce document HTML, cette mise en forme est obtenue via la commande MathJax `\boldsymbol{}` (équivalent de `\bm{}` du package LaTeX `{bm}` utilisé dans la version PDF).

- Un scalaire s'écrit $x$ ou $\mu$
- Un vecteur s'écrit $\boldsymbol{x}$ ou $\boldsymbol{\mu}$ (commande `\boldsymbol{x}`)
- Une matrice s'écrit $\boldsymbol{\Sigma}$ (commande `\boldsymbol{\Sigma}`)

Cette convention, universelle en statistique multivariée et en algèbre linéaire, permet de distinguer immédiatement la nature (scalaire, vecteur, matrice) de chaque objet mathématique.
:::
...
- $\boldsymbol{\mu} = (\mu_1, \ldots, \mu_p)^\top$ : le vecteur de moyennes (centre du nuage de points).
...
::: {.callout-note}
### Rappel : la matrice identité $I_2$ pour $p = 2$

Dans notre simulation bivariée ($p = 2$), les groupes 1 et 2 ont une matrice de covariance *sphérique*, c'est-à-dire la *matrice identité* $I_2$ :

$$I_2 = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}$$

Cette matrice signifie que les deux variables $V_1$ et $V_2$ ont chacune une variance unitaire ($\sigma^2 = 1$) et sont *indépendantes* (covariance nulle). La loi $\mathcal{N}_2(\boldsymbol{\mu}, I_2)$ génère ainsi des nuages de points circulaires, sans corrélation ni orientation préférentielle — c'est la forme que le K-means détecte le mieux.
:::
...
1. **Initialisation** : placer $K$ centroïdes au hasard dans l'espace des données.
2. **Attribution** : affecter chaque observation au centroïde le plus proche — $d(\boldsymbol{x}, \boldsymbol{\mu}_k) = \lVert \boldsymbol{x} - \boldsymbol{\mu}_k \rVert$.
3. **Mise à jour** : recalculer chaque centroïde comme la moyenne arithmétique de son groupe.
4. **Convergence** : répéter les étapes 2–3 jusqu'à ce qu'aucune observation ne change de groupe.

La figure ci-dessous, extraite de @lebart1995, illustre graphiquement ces quatre étapes pour $K = 2$ groupes. On y observe, de haut en bas : le tirage aléatoire des deux centres initiaux ($C_1^0$ et $C_2^0$) ; la constitution des deux premières classes ($I_1^0$ et $I_2^0$) délimitées par la frontière médiane perpendiculaire au segment joignant les deux centres ; le déplacement des barycentres ($C_1^1$, $C_2^1$) vers le centre de masse de chaque classe ; puis la convergence vers une partition finale stable ($I_1^2$, $I_2^2$). La frontière de séparation (trait oblique) se déplace à chaque itération jusqu'à ce que les classes ne changent plus.

![Illustration de l'algorithme K-means ($K = 2$) en quatre étapes successives : initialisation aléatoire des centres ($C_1^0$, $C_2^0$), formation des classes ($I_1^0$, $I_2^0$), recalcul des barycentres et convergence vers la partition finale. *Source : @lebart1995.*](dessin_algo_kmeans.png){#fig-kmeans-algo fig-align="center" width="70%"}
...
::: {.callout-note}
### Analogie : du déterministe au probabiliste
Le K-means affecte chaque observation à *un seul* groupe de façon déterministe :
"Tu appartiens au groupe 2, point final." Le GMM est plus nuancé : "Tu
appartiens au groupe 2 avec une probabilité de 78 %, au groupe 1 avec 18 %,
au groupe 3 avec 4 %." Cette *appartenance probabiliste* est à la fois plus
réaliste et plus informative.
:::
...
par un poids $\pi_k$, un centre $\boldsymbol{\mu}_k$ et une dispersion
$\boldsymbol{\Sigma}_k$. La densité de mélange s'écrit :

$$f(x_i) = \sum_{k=1}^{K} \pi_k \,
\mathcal{N}_p(x_i \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)$$

avec $\sum_{k=1}^K \pi_k = 1$ et $\pi_k > 0$.

### L'algorithme EM

Les paramètres $\Theta = \{\pi_k, \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k\}_{k=1}^K$ sont
estimés par maximum de vraisemblance via l'algorithme *EM* (Expectation-Maximisation) [@dempster1977],
qui alterne deux étapes jusqu'à convergence.

*Étape E (Expectation)* — calcul des responsabilités $r_{ik}$ :

$$r_{ik} = \frac{\pi_k \cdot \mathcal{N}_p(x_i\mid\boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)}
{\displaystyle\sum_{j=1}^{K} \pi_j \cdot
\mathcal{N}_p(x_i\mid\boldsymbol{\mu}_j, \boldsymbol{\Sigma}_j)}$$

$r_{ik} \in [0,1]$ est la probabilité *a posteriori* que l'observation $x_i$
provienne de la composante $k$ (application directe de la règle de Bayes).
On a $\sum_k r_{ik} = 1$ pour tout $i$ : là où le K-means donne une appartenance
dure (0 ou 1), le GMM produit un vecteur complet de probabilités (*soft assignment*).
...
\hat{\boldsymbol{\mu}}_k = \frac{1}{n_k}\sum_i r_{ik}\, x_i, \qquad
\hat{\boldsymbol{\Sigma}}_k = \frac{1}{n_k}\sum_i
r_{ik}(x_i - \hat{\boldsymbol{\mu}}_k)(x_i - \hat{\boldsymbol{\mu}}_k)^\top$$

::: {.callout-tip title="À retenir"}
La log-vraisemblance $\ell(\Theta) = \sum_i \ln f(x_i)$ est *garantie non
décroissante* à chaque itération EM. L'algorithme s'arrête quand l'incrément
$|\ell^{(t+1)} - \ell^{(t)}| < \varepsilon$ (défaut : $10^{-5}$ dans `mclust`).
En pratique, on relance depuis plusieurs initialisations pour éviter les maxima locaux.
:::
...
$\lambda_k$ contrôle le *volume*, $D_k$ l'*orientation*, $A_k$ la *forme*
($\det A_k = 1$). En contraignant (E = Égal entre groupes) ou libérant
(V = Variable) chacune de ces trois composantes, `mclust` définit 14 familles
de modèles (EII, VII, EEI, EEE, …, VVV).

### La vraisemblance : mesurer l'adéquation du modèle

La *log-vraisemblance* $\ell(\Theta)$ mesure à quel point le modèle ajusté
"colle" aux données observées :

$$\ell(\Theta) = \sum_{i=1}^{n} \ln\!\left[
\sum_{k=1}^{K} \pi_k \, \mathcal{N}_p(x_i \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)
\right]$$

::: {.callout-note}
### Intuition de la vraisemblance
Imaginez 600 élèves répartis dans 3 classes. La vraisemblance répond à :
*"Quelle configuration de classes (centres, dispersions, proportions) aurait
le plus de chances de produire exactement ces 600 élèves ?"* Plus $\ell$ est
grand (moins négatif), mieux le modèle explique les données. L'algorithme EM
cherche les paramètres qui maximisent $\ell$.

Chaque terme $\ln[\ldots]$ est négatif (log d'une probabilité $< 1$), donc
$\ell < 0$ en général. Un modèle avec plus de composantes $K$ ou plus de
paramètres libres atteint toujours une vraisemblance au moins aussi élevée
--- d'où la nécessité de la pénalisation du BIC.
:::

### Les degrés de liberté : le coût de la complexité

Le paramètre $d$ dans la formule du BIC est le *nombre de paramètres libres*
du modèle. Pour les données de Flynt & Dean ($p = 2$ variables, $K = 3$ groupes) :

| Composante | Formule | Valeur |
|---|---|---|
| Proportions $\pi_k$ | $K - 1$ | 2 |
| Moyennes $\boldsymbol{\mu}_k$ | $K \times p$ | 6 |
| Covariances (selon modèle) | variable | dépend de la famille |

Un modèle VVV (totalement libre) utilise $p(p+1)/2 = 3$ paramètres par
matrice de covariance, soit $3 \times 3 = 9$ supplémentaires, pour un total
de $d = 17$. Des familles plus contraintes (EEE, EVE…) partagent des
paramètres entre groupes et comptent moins de degrés de liberté.

::: {.callout-tip title="À retenir"}
Le modèle sélectionné par `mclust` expose son $d$ via `mc_model_v2$df` en R.
Sur $n = 600$ observations, $\ln(600) \approx 6{,}4$ : chaque paramètre
supplémentaire coûte environ *6,4 points de BIC*, ce qui rend la
pénalisation très efficace pour écarter les modèles sur-paramétrés.
:::

### Sélection automatique via le BIC

`mclust` [@fraley2002; @scrucca2016] sélectionne simultanément $K$ et la
famille de covariance en maximisant le BIC de Fraley-Raftery [@schwarz1978] :

$$\text{BIC} = \underbrace{2\ln\hat{L}}_{\text{ajustement}} -
\underbrace{d\ln n}_{\text{pénalité complexité}}$$

::: {.callout-warning}
*Convention inversée.* Dans la littérature standard, le BIC se *minimise*
($-2\ln\hat{L} + d\ln n$). Dans `mclust`, il se *maximise* — signe opposé,
même principe de pénalisation de la complexité. Ne pas confondre les deux
conventions en comparant des sources différentes.
:::
...
kable(bic_top5,
      ...
      caption   = "Cinq meilleurs modèles selon le BIC de Fraley-Raftery (plus élevé = meilleur).") |>
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) |>
...
#| fig-cap: "Classification Mclust (G=3) : chaque individu est coloré selon son groupe d'appartenance identifié par l'algorithme."
...
::: {.callout-note}
### Lecture du tableau
Les trois méthodes sont évaluées sur les mêmes variables continues $(V_1, V_2)$ et la même vérité terrain, ce qui rend les ARI directement comparables. Mclust domine grâce à sa modélisation elliptique, tandis que la CHA souffre de ses fusions irréversibles (30% d'erreur). Pour une présentation complète des mesures de qualité des partitions — indice de Rand ajusté, silhouette, indice de Calinski-Harabasz, V de Cramer et information mutuelle — on se reportera à @rakotomalala2024.
:::
...
if (length(timer_lignes) > 0) {
  ...
  if (!is.null(df_timers) && nrow(df_timers) > 0) {
    ...
    duree_knitr_num <- tryCatch(
      get(".prev_knitr_total_html", envir = .GlobalEnv),
      error = function(e) NA_real_
    )
...
# KNITR_TOTAL de la compilation précédente (stocké dans setup avant écrasement)
duree_p2 <- tryCatch(
  get(".prev_knitr_total_html", envir = .GlobalEnv),
  error = function(e) NA_real_
)
...
Au lancement de chaque compilation, le chunk `setup` lit le fichier `compilation_stats_html.txt` *avant* de l'écraser, et sauvegarde en mémoire le `[KNITR_TOTAL]` de la session précédente. Cette valeur est ainsi disponible *dès la première compilation* qui suit une compilation complète. Les tableaux affichent donc : la somme des chunks R de la compilation *en cours*, et le total knitr de la compilation *précédente* — ce qui est la seule approche fiable, puisque `timer-final-html` écrit ce total *après* que tous les tableaux ont été générés.
:::
