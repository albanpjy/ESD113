...
library(broom)
library(factoextra)
library(tinytable)   # Tables multi-format : HTML / LaTeX / Typst
...
.prev_knitr_total_typst <- tryCatch({
  lignes_prev <- readLines("compilation_stats_typst.txt")
...
assign(".prev_knitr_total_typst", .prev_knitr_total_typst, envir = .GlobalEnv)
...
::: {style="text-align: right; margin-top: 1.5em; font-style: italic;"}
*Alban VIDELOUP*\
...
Ce document est la version **Typst** du rapport. Typst offre une composition typographique de haute qualité, comparable à LaTeX, tout en assurant des temps de compilation nettement plus rapides. Les formules mathématiques sont rendues nativement par le moteur Typst ; les callouts Quarto, les tableaux et les graphiques R sont pleinement pris en charge. Pour chaque formule, le bloc grisé affiche le **code LaTeX source** à titre pédagogique, et le rendu apparaît immédiatement en dessous.
:::
...
Pour chaque formule, le bloc grisé montre le **code LaTeX source** tel qu'il apparaît dans le document `.qmd`, et le rendu est affiché juste en dessous. Cette double présentation est précieuse pour l'apprentissage : on voit à la fois ce qu'on écrit et ce que Typst produit.
...
![Tableau officiel FDJ — toutes grilles de 2 à 10 numéros (version 2020)](nouveau-rapport-de-gains-keno-2020.webp){width="80%" fig-align="center"}
...
Avec K-means et la CHA, chaque individu est assigné à exactement un groupe, sans la moindre nuance. Les **Modèles de Mélanges Gaussiens** (GMM) apportent une vision plus honnête : chaque individu appartient à chaque groupe avec une certaine probabilité. Formellement :
...
Mclust [@fraley2002; @scrucca2016] sélectionne automatiquement $K$ et la structure de covariance en maximisant le **Critère d'Information Bayésien (BIC)** [@schwarz1978] :
...
| **2 — knitr (R)** | knitr + R | Exécute tous les chunks. `proc.time()` lancé au `setup`, arrêté au `timer-final-typst`. | ✅ |
| **3 — Rendu Typst** | pandoc + typst | Convertit le `.md` en `.pdf` via le moteur Typst | ❌ |
...
À la **première compilation**, `[KNITR_TOTAL]` n'est pas encore dans le fichier (écrit par `timer-final-typst` *après* les tableaux). **Dès la deuxième compilation**, toutes les valeurs apparaissent — le fichier `compilation_stats_typst.txt` contient les données de la session précédente.
:::

## Environnement système {.unnumbered}

```{r tbl-sysinfo-typst}
#| label: tbl-sysinfo-typst
#| echo: false
...
```{r tbl-timers-typst}
#| label: tbl-timers-typst
#| echo: false

lignes       <- tryCatch(readLines("compilation_stats_typst.txt"), error = function(e) character(0))
...
if (length(timer_lignes) > 0) {
  ...
  if (!is.null(df_timers) && nrow(df_timers) > 0) {
    ...
    duree_knitr_num <- tryCatch(
      get(".prev_knitr_total_typst", envir = .GlobalEnv),
      error = function(e) NA_real_
    )
    ...
    note_bas <- if (!is.na(duree_knitr_num)) {
      ...
      sprintf("Somme chunks R : %.2f sec | Phase knitr globale : disponible a la 2e compilation",
              total_chunks)
    }

    df_timers$Signal <- ifelse(df_timers$Secondes >= 5, "LENT",
                        ifelse(df_timers$Secondes >= 1, "MOYEN", "OK"))

    df_timers[, c("Rang", "Signal", "Chunk", "Secondes")] |>
      setNames(c("Rang", "Statut", "Chunk R", "Duree (sec)")) |>
      tt(caption = note_bas) |>
      style_tt(j = 1:2, align = "c") |>
      style_tt(j = 3, monospace = TRUE) |>
      style_tt(j = 4, bold = TRUE, align = "r") |>
      style_tt(i = which(df_timers$Secondes >= 5), background = "#fff3cd")
  } else {
    cat("Aucune donnée timer parsée.")
  }
} else {
  cat("Fichier compilation_stats_typst.txt absent — relancez une compilation complète.")
}
```
...
```{r tbl-bilan-typst}
#| label: tbl-bilan-typst
#| echo: false

lignes2 <- tryCatch(readLines("compilation_stats_typst.txt"), error = function(e) character(0))
...
nd <- "(disponible a la 2e compilation)"

bilan_typst <- data.frame(
  Phase  = c("Phase 1 -- Quarto / pandoc",
             "Phase 2a -- Execution chunks R (somme)",
             "Phase 2b -- Overhead knitr (I/O, rendus)",
             "Phase 2 -- Total knitr mesure",
             "Phase 3 -- Rendu Typst (pandoc)"),
  Mesure = c(
    "Non mesurable",
  ...
  Note = c("~2-5 sec estimees", "Mesure chunk par chunk",
           "Rendu ggplot2, I/O fichiers", "Seule valeur exacte",
           "Generalement < 10 sec pour Typst"),
  stringsAsFactors = FALSE
)

bilan_typst |>
  setNames(c("Phase", "Durée mesurée", "Remarque")) |>
  tt() |>
  style_tt(j = 1, align = "l") |>
  style_tt(j = 2, bold = TRUE, align = "r") |>
  style_tt(j = 3, align = "l") |>
  style_tt(i = 4, background = "#d4edda")
```

::: callout-tip
## Comment fonctionne le timer ?

Au lancement de chaque compilation, le chunk `setup` lit le fichier `compilation_stats_typst.txt` *avant* de l'écraser, et sauvegarde en mémoire le `[KNITR_TOTAL]` de la session précédente. Cette valeur est ainsi disponible **dès la première compilation** qui suit une compilation complète. Les tableaux affichent donc : la somme des chunks R de la compilation *en cours*, et le total knitr de la compilation *précédente* --- ce qui est la seule approche fiable, puisque `timer-final-typst` écrit ce total *après* que tous les tableaux ont été générés.
:::

```{r timer-final-typst}
#| label: timer-final-typst
#| echo: false
#| include: false
